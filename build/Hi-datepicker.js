import { Component, ViewContainerRef, Input, Output, EventEmitter, forwardRef } from "@angular/core";
import { NG_VALUE_ACCESSOR } from "@angular/forms";
import * as moment from 'moment-jalaali';
export var CALENDAR_VALUE_ACCESSOR = {
    provide: NG_VALUE_ACCESSOR,
    useExisting: forwardRef(function () { return HiDatePickerComponent; }),
    multi: true
};
var HiDatePickerComponent = (function () {
    function HiDatePickerComponent(viewContainerRef) {
        //====================Output==================
        this.changeDate = new EventEmitter();
        //============================================
        this._date = moment();
        this._selectedDate = moment();
        this.timeData = { hour: 10, minute: 20 };
        this.days = [];
        this.type = "Gregorian";
    ...
    Object.defineProperty(HiDatePickerComponent.prototype, "minDate", {
        set: function (date) {
            this._minDate = date;
    ...
    Object.defineProperty(HiDatePickerComponent.prototype, "maxDate", {
        set: function (date) {
            this._maxDate = date;
            this.generateCalendar();
        },
    ...
    HiDatePickerComponent.prototype.ngOnInit = function () {
        var _this = this;
        this.selectedDate = moment();
        var currentTime = { hour: this.selectedDate.hour(), minute: this.selectedDate.minute() };
    ...
    Object.defineProperty(HiDatePickerComponent.prototype, "value", {
        get: function () {
            return this._date;
        },
        set: function (value) {
    ...
    Object.defineProperty(HiDatePickerComponent.prototype, "selectedDate", {
        get: function () {
    ...
    Object.defineProperty(HiDatePickerComponent.prototype, "year", {
        get: function () {
            switch (this.type) {
                case "Solar":
    ...
    HiDatePickerComponent.prototype.subtract = function (count, type) {
        this.value = this.value.subtract(count, type);
        this.generateCalendar();
    };
    HiDatePickerComponent.prototype.add = function (count, type) {
        this.value = this.value.add(count, type);
        this.generateCalendar();
    };
    Object.defineProperty(HiDatePickerComponent.prototype, "hour", {
        get: function () {
            if (!this.offAMOrPM) {
    ...
    Object.defineProperty(HiDatePickerComponent.prototype, "minute", {
        get: function () {
    ...
    HiDatePickerComponent.prototype.changeTime = function () {
        var currentDay = this.days.filter(function (day) {
            if (day.selected == true)
                return true;
        })[0];
        if (currentDay == undefined)
            currentDay = this.days.filter(function (day) {
                if (day.today == true)
                    return true;
            })[0];
        if (currentDay == undefined) {
    ...
    HiDatePickerComponent.prototype.generateCalendar = function () {
        switch (this.type) {
            case "Solar":
    ...
    HiDatePickerComponent.prototype.jalaliGenerateCalendar = function () {
        var date = moment(this._date);
        var month = date.jMonth();
        var year = date.jYear();
        var n = 1;
        var firstWeekDay = date.jDate(1).day();
        var lastDayMonth = moment.jDaysInMonth(year, month);
        ...
        var dd = (lastDayMonth + (Math.abs(n) + 1)) % 7;
        var dayInit = dd > 0 ? lastDayMonth + (7 - dd) : lastDayMonth;
        this.days = [];
        for (var i = n; i <= dayInit; i += 1) {
            var currentDate = moment(i + "." + (month + 1) + "." + year, 'jDD.jMM.jYYYY');
            var today = (moment().isSame(currentDate, 'day') && moment().isSame(currentDate, 'month')) ? true : false;
            var selected = (this.selectedDate && this.selectedDate.isSame(currentDate, 'day')) ? true : false;
    ...
    HiDatePickerComponent.prototype.checkValidDate = function (d) {
        if (this.checkChecker(this._maxDate) && this.checkChecker(this._minDate)) {
            if (this.validMinDate(d) && this.validMaxDate(d))
                return d;
            else {
                d.enabled = false;
                return d;
    ...
    HiDatePickerComponent.prototype.checkChecker = function (d) {
        if (d.year == 0 && d.month == 0 && d.day == 0)
            return false;
        return true;
    };
    HiDatePickerComponent.prototype.validMaxDate = function (d) {
        if (d.year < this._maxDate.year)
            return true;
        else if (d.year == this._maxDate.year && d.month < this._maxDate.month)
            return true;
        else if (d.year == this._maxDate.year && d.month == this._maxDate.month && d.day <= this._maxDate.day)
            return true;
        else
            false;
    };
    HiDatePickerComponent.prototype.validMinDate = function (d) {
        if (d.year > this._minDate.year)
            return true;
        else if (d.year == this._minDate.year && d.month > this._minDate.month)
            return true;
        else if (d.year == this._minDate.year && d.month == this._minDate.month && d.day >= this._minDate.day)
            return true;
        else
            false;
    };
    HiDatePickerComponent.prototype.miladiGenerateCalendar = function () {
        var date = moment(this._date);
        var month = date.month();
        var year = date.year();
        var n = 1;
        var firstWeekDay = date.date(1).day();
        var lastDayMonth = date.daysInMonth();
        n = (firstWeekDay - 1) * -1;
        var dayInit = lastDayMonth + Math.abs(n);
        this.days = [];
        for (var i = n; i <= dayInit; i += 1) {
            var currentDate = moment(i + "." + (month + 1) + "." + year, 'DD.MM.YYYY');
            var today = (moment().isSame(currentDate, 'day') && moment().isSame(currentDate, 'month')) ? true : false;
            var diff = this.selectedDate.diff(currentDate, 'day');
            var selected = (diff == 0) ? true : false;
            if (i > 0 && i <= lastDayMonth) {
                this.days.push({
                    day: i,
                    month: month + 1,
    ...
    HiDatePickerComponent.prototype.selectDate = function (e, i, enabled) {
        if (enabled) {
            e.preventDefault();
    ...
    HiDatePickerComponent.prototype.prevMonth = function () {
        switch (this.type) {
            case "Solar":
    ...
    HiDatePickerComponent.prototype.nextMonth = function () {
        switch (this.type) {
            case "Solar":
    ...
    HiDatePickerComponent.prototype.writeValue = function (value) {
    ...
    HiDatePickerComponent.prototype.registerOnChange = function (fn) {
        this.onChange = fn;
    };
    HiDatePickerComponent.prototype.registerOnTouched = function (fn) {
        this.onTouched = fn;
    };
    HiDatePickerComponent.prototype.toggle = function () {
        if (!this.alwaysOpened)
            this.opened = !this.opened;
    };
    HiDatePickerComponent.prototype.open = function () {
        this.opened = true;
    };
    HiDatePickerComponent.prototype.close = function () {
        if (!this.alwaysOpened)
            this.opened = false;
    };
    HiDatePickerComponent.prototype.AMPM = function () {
        if (this.AMOrPM == 'am') {
            this.AMOrPM = 'pm';
    ...
    HiDatePickerComponent.prototype.aro = function () {
        this.selectedDate = moment();
    ...
    return HiDatePickerComponent;
}());
export { HiDatePickerComponent };
HiDatePickerComponent.decorators = [
    { type: Component, args: [{
                moduleId: module.id,
...
/** @nocollapse */
HiDatePickerComponent.ctorParameters = function () { return [
    { type: ViewContainerRef, },
]; };
HiDatePickerComponent.propDecorators = {
    'class': [{ type: Input },],
    'expanded': [{ type: Input },],
    'opened': [{ type: Input },],
    'format': [{ type: Input },],
    'viewFormat': [{ type: Input },],
    'firstWeekdaySaturday': [{ type: Input },],
    'persianCalendar': [{ type: Input },],
    'time': [{ type: Input },],
    'alwaysOpened': [{ type: Input },],
    'hiddenButton': [{ type: Input },],
    'offAMOrPM': [{ type: Input },],
    'minDate': [{ type: Input },],
    'maxDate': [{ type: Input },],
    'initTime': [{ type: Input },],
    'changeDate': [{ type: Output },],
};
//# sourceMappingURL=Hi-datepicker.js.map